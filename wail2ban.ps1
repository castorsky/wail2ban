...
function help { 
  ...
  function Read-BlockList {
	switch ($BLOCK_TYPE) {
	  "NETSH" {
		...
		return $firewall.rules | Where-Object { $_.name -match $FirewallRulePrefix } | Select-Object name, description
	  }
	  Default {
		Write-LogError "Don't have a known Block Type. $BLOCK_TYPE"
	  }
	}  
  }
  ...
  function Test-RuleExists ($IP) { 
	switch($BLOCK_TYPE) {
	  "NETSH" {
		$getRulesCommand = "netsh advfirewall firewall show rule name=`"$FirewallRulePrefix $IP`""
	  }
	  Default { Write-LogError "Don't have a known Block Type. $BLOCK_TYPE" }
	}
	if ($getRulesCommand) { 
	  $result = Invoke-Expression $getRulesCommand
	  # Когда Windows выводит существующее правило, отображаются минусы
	  if ($result -match "----------") {
		return $true
	  }  else { 
		return $false
  ...
  function Read-BlockPeriodLog { 
	...
	} else { Write-LogDebug "No IPs to collect from BannedIPLog" }
  } 
  ...
  function Block-Address ($IP, $expireDate) {
	...
	if ($result) {
	  ...
	  if (Test-RuleExists $IP) { Write-LogWarning ("IP $IP already blocked.")
	  } else {
  ...
  function Remove-FirewallRule ($IP) { 
	switch($BLOCK_TYPE) {
	  "NETSH" { $removeRuleCommand = "netsh advfirewall firewall delete rule name=`"$FirewallRulePrefix $IP`""}
	  default { Write-LogError "Don't have a known Block Type. $BLOCK_TYPE" }
	}	 
	if ($removeRuleCommand) { 
	  $result = Invoke-Expression $removeRuleCommand
	  if ($LASTEXITCODE -eq 0) {
  ...
  function Unblock-ExpiredRecords {
	...
	if ($currentBlockList) { 
	  foreach ($record in $currentBlockList) { 		
		$IP = $record.Name.substring($FirewallRulePrefix.length+1)
		$releaseDate = $record.Description.substring("Expire: ".Length)
  ...
  function Open-MikrotikConnection {
	...
	$mikroConnection = & $execCmd @execParams
	if (!$mikroConnection) {
  ...
  Read-BlockPeriodLog
  if ($BLOCK_TYPE -eq "MIKROTIK") { Open-MikrotikConnection }
